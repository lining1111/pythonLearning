...
随着系统的功能增加，服务会越来越多，监听的端口也会越来越多，为了系统的稳定性，一般而言会有国际惯例来约定端口资源，也会利用容器技术将系统进行软隔离。
...
如果没有配置网络插件时，STATUS为NotReady的，配置完flannel后就好了
...
kubectl get pod -o wide可以看到是运行在哪个节点，这点可以看运行的端口是哪个，然后在浏览器输入任意的node的ip加上这个端口就可以打开nginx
...
在系统有的情况下，1个master和多个node，关闭了系统防火墙和swap，按照角色设置角色名称
1、为apiserver和etcd自签证书
    cfssl是一个开源的证书管理工具，使用json文件生成证书，相比openssl更方便使用

2、部署etcd集群
    etcd是分布式键值存储系统，用于服务发现和管理配置，使用golang编写
    service etcd status
3、部署master组件
kube-apiserver
